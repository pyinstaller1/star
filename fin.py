...
def get_processed_stock_data(year, q):
    ...
    # 실제 환경에서는 종목별 API 호출을 통해 업종을 가져오세요
    industry = "전선 및 케이블 제조업" 
    return {"raw": raw, "metrics": metrics, "industry": industry}
...
# 2. 출력 로직 (업종 추가)
print(f"{'분기':<8} | {'업종':<12} | {'매출액':<14} | {'영업이익':<14} | {'영업현금흐름':<14}")
print("-" * 95)
for key in sorted_keys:
    d, ind = all_data[key]['raw'], all_data[key]['industry']
    print(f"{key:<8} | {ind:<12} | {d.get('매출액',0):>13,} | {d.get('영업이익',0):>13,} | {d.get('영업활동현금흐름',0):>13,}")
...
for i, key in enumerate(sorted_keys):
    ...
    r_yoy = calc_change(r, prev_yoy['raw'].get('매출액', 0) if prev_yoy else None)
    o_yoy = calc_change(o, prev_yoy['raw'].get('영업이익', 0) if prev_yoy else None)
    a_yoy = calc_change(a, prev_yoy['raw'].get('자산총계', 0) if prev_yoy else None)
    r_qoq = calc_change(r, prev_qoq['raw'].get('매출액', 0) if prev_qoq else None)
    o_qoq = calc_change(o, prev_qoq['raw'].get('영업이익', 0) if prev_qoq else None)
    a_qoq = calc_change(a, prev_qoq['raw'].get('자산총계', 0) if prev_qoq else None)
